...
// Cache break: the cache-hit ratio collapses from healthy to near-zero and
// STAYS collapsed on the next turn. The persistence check is essential — a
// one-turn dip is normal whenever a large new chunk (a slash command, a big
// file, a fresh user message) enters context, and must not be flagged. Fires
...
function detectCacheBreak(turns, opts) {
  ...
  for (let i = 1; i < ratios.length; i++) {
    ...
    if (prev != null && cur != null && prev > o.cacheHigh && cur < o.cacheLow && next != null && next < o.cacheLow) {
